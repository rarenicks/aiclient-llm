...
async def test_concurrency_flaw_reproduction():
    ...
    # Price: Input $1.5/1M. 1000 tokens = $0.0015
    ...
    # Expected total: 0.03 (A) + 0.0015 (B) = 0.0315
    # If bug exists (context clobbered by B): 0.0015 (A as B) + 0.0015 (B) = 0.0030

    # We verify it's close to 0.0315
    expected = 0.0315
    tolerance = 0.0001

    assert abs(cost_tracker.total_cost_usd - expected) < tolerance, \
        f"Race condition or calculation error! Expected ~{expected}, \
        got {cost_tracker.total_cost_usd:.6f}"
